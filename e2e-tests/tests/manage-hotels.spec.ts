...
test('should allow user to add a hotel', async ({ page }) => {
  ...
  await page.setInputFiles('[name="imageFiles"]', [
    path.join(__dirname, "files", "image-1.jpg"),
    path.join(__dirname, "files", "image-2.jpg")
  ])
...
test('should display hotels', async ({ page }) => {
  ...
  await expect(page.getByText("Trần Huy Linh")).toBeVisible()
...
test("should edit hotel", async ({page}) => {
  ...
  await expect(page.locator('[name="name"]')).toHaveValue("Trần Huy Linh")
  await page.locator('[name="name"]').fill("Trần Huy Linh Updated")
  ...
  await expect(page.locator('[name="name"]')).toHaveValue("Trần Huy Linh Updated")
  await page.locator('[name="name"]').fill("Trần Huy Linh")
  await page.getByRole("button", {name: "Save"}).click()
})
